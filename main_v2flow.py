...
from models import V2Flow
from engine_v2flow import train_one_epoch, evaluate
import copy
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='v2flow_large', type=str, metavar='MODEL',
                        help='Name of model to train')

    # VAE parameters
    parser.add_argument('--img_size', default=1024, type=int,
                        help='images input size')
    parser.add_argument('--vae_path', default="mit-han-lab/dc-ae-f32c32-sana-1.1-diffusers", type=str,
                        help='images input size')
    parser.add_argument('--vae_embed_dim', default=32, type=int,
                        help='vae output embedding dimension')
    parser.add_argument('--vae_stride', default=32, type=int,
    ...
    parser.add_argument('--save_last_freq', type=int, default=3, help='save last frequency')
    ...
    parser.add_argument('--flowloss_d', type=int, default=8)
    parser.add_argument('--flowloss_w', type=int, default=1536)
    ...
    parser.add_argument('--data_path', default='./data/imagenet', type=str,
                        help='dataset path')
    parser.add_argument('--class_num', default=16384, type=int)
    ...
    parser.add_argument('--cached_path', default='/nfs-134/zhangguiwei/cache_dcae', help='path to cached latents')
...
def main(args):
   ...
   # vae = AutoencoderKL(embed_dim=16, ch_mult=(1, 1, 2, 2, 4), ckpt_path=args.vae_path).cuda().eval()
    from diffusers import AutoencoderDC
    vae = AutoencoderDC.from_pretrained(args.vae_path, torch_dtype=torch.float32).cuda().eval()
    for param in vae.parameters():
        param.requires_grad = False

    model = V2Flow.__dict__[args.model](
        img_size=args.img_size,
        vae_stride=args.vae_stride,
        downsampler_hidden_dim=args.downsampler_hidden_dim,
        visual_vocabulary=args.visual_vocabulary,
        visual_codebook_size=args.visual_codebook_size,
        ...
        flowloss_d=args.flowloss_d,
        flowloss_w=args.flowloss_w,
    ...
    # no weight decay on bias, norm layers, and flowloss MLP
    ...
    if args.resume and os.path.exists(os.path.join(args.resume, "checkpoint-last.pth")):
        ...
        checkpoint_new=checkpoint['model']
        for key,val in model_without_ddp.state_dict().items():
            '''
            if 'remain' in key or 'projector' in key:
                print(key)
                checkpoint_new[key]=val
            '''
            if key not in list(checkpoint_new.keys()):
                checkpoint_new[key]=val
            if val.shape!=checkpoint_new[key].shape:
                checkpoint_new[key]=val
                print("error")
        model_without_ddp.load_state_dict(checkpoint_new,strict=False)
        model_params = list(model_without_ddp.parameters())
        ema_state_dict = checkpoint['model_ema']
        ema_params=[]
        for name, _ in model_without_ddp.named_parameters():
            if name in ema_state_dict.keys() and ema_state_dict[name].shape==model_without_ddp.state_dict()[name].shape:
                ema_params.append(ema_state_dict[name].cuda())
            else:
                ema_params.append(model_without_ddp.state_dict()[name].cuda())
